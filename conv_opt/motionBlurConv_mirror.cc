...
{  mxArray       *matrix_X, *matrix_bmag, *matrix_bori;
   ...
   // create matrix for tempary usage
   double* ker = (double*)malloc(sizeof(double) * 25 * 25); // maximum size of kernel
           ...
           for(l = -sy; l <= sy; l++) // y
           {
              ...
              //    }
                 ...
                 if (sumKernel > 0)
                 {
                    ...
                    R[j * w_img + i + offSet_img] += ker[(l + sy) * (2*sx + 1) + k + sx] / sumKernel;
                 }
                 else
                 {
                    R[j * w_img + i + offSet_img] += ker[(l + sy) * (2*sx + 1) + k + sx];
                 }

              //  if (R[185064] > 0)
              //  {
              //     printf(" !(%d %d %d %d %f %f %f %d)", off_x, off_y, i, j, ker[(l + sy) * (2*sx + 1) + k + sx], sumKernel, R[j * w_img + i + offSet_img], j * w_img + i + offSet_img);
              //  }
                
                
                 //if(j * w_img + i + offSet_img == )
                 
                  //if((j * w_img + i + offSet_img == y0 * w_img + x0)) // j == y0 & i == x0 - 1 & // (j * w_img + i + offSet_img == y0 * w_img + x0)//
                  //{
                  //  printf(" (%d %d %d %d %f %f %f)", off_x, off_y, i, j, ker[(l + sy) * (2*sx + 1) + k + sx], sumKernel, R[j * w_img + i + offSet_img]);
                  //}
              }
           }
